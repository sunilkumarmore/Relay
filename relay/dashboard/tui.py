...
def left_panel(status: dict, store: Store) -> Panel:
    ...
    sessions_by_id = {s.get("session_id"): s for s in store.list_sessions()}
    for worker in status.get("active_workers", []):
        ...
        worker_table.add_row(
            worker.get("worker_id", "?"), worker.get("session_id", "?"), f"{bar} {done}/{total}"
        )
    ...
    for call in status.get("recent_calls", [])[-5:]:
        ...
        recent.add_row(
            ts,
            str(call.get("worker_id", "")),
            f"{call.get('latency_ms', 0)} ms",
            "yes" if call.get("success") else "no",
        )
...
def right_panel(store: Store) -> Panel:
...
def build_layout(status: dict, store: Store) -> Layout:
    layout = Layout()
    layout.split_column(
    ...
    layout["left"].update(left_panel(status, store))
    layout["right"].update(right_panel(store))
...
def main() -> int:
    config.load_env()
    inference_registry = (
        config.get("INFERENCE_REGISTRY", "http://localhost:8765") or "http://localhost:8765"
    )
    store = get_store()
    ...
    with Live(refresh_per_second=4, screen=False) as live:
        while True:
            ...
            live.update(build_layout(status, store))
            time.sleep(2)
